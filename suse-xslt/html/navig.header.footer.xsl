...
<xsl:template match="part" mode="bgnavi">
  ...
  <span>
    <xsl:choose>
        <xsl:when
          test="generate-id(.) = generate-id($node/ancestor-or-self::part)">
...
<xsl:template match="chapter" mode="bgnavi">
  ...
  <xsl:variable name="num">
    ...
    <span>
      <xsl:choose>
        <xsl:when
          test="generate-id(.) = generate-id($node/ancestor-or-self::chapter)">
...
<xsl:template match="article" mode="bgnavi">
  ...
  <span>
    <xsl:choose>
      <xsl:when
        test="generate-id(.) = generate-id($node/ancestor-or-self::sect1)">
...
<xsl:template name="generate.link.line">
    ...
    <xsl:if test="$suppress.navigation = '0' and
                  $suppress.header.navigation = '0' and
                  not(self::set)">
      ...
      <div class="nav{$orientation}">
        ...
        <table width="100%" summary="Navigation {$orientation}" border="0" 
          class="bctable">
          <tr>
            <td width="80%">
              ...
              <xsl:choose>
                <xsl:when test="count(following-sibling::*[1])">
                  ...
                  <a accesskey="n">
                    ...
                    <span><xsl:copy-of select="$breadcrumbs.next"
                      /></span>
                    <!--<xsl:call-template name="navig.content">
