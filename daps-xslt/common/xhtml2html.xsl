...
<xsl:template match="/">
  <xsl:comment>This file was created automatically by xhtml2html</xsl:comment>
  <xsl:apply-templates/>
</xsl:template>
...
<xsl:template match="xsl:stylesheet" >
  <!--<xsl:variable name="a">
    ...
    <xsl:copy>
      ...
      <xsl:copy-of select="namespace::*[namespace-uri(.) != 'http://www.w3.org/1999/xhtml']"/>
      <xsl:apply-templates/>
   </xsl:copy>
</xsl:template>
  ...
  <xsl:copy>
    <xsl:attribute name="href">
      <xsl:call-template name="string.subst">
        ...
        <xsl:with-param name="replacement">/html/</xsl:with-param>
      </xsl:call-template>
    </xsl:attribute>
  </xsl:copy>
</xsl:template>-->
...
<xsl:template match="xsl:param[@name='stylesheet.result.type']">
  <xsl:copy>
    <xsl:copy-of select="@*"/>
    <xsl:attribute name="select">'html'</xsl:attribute>
  </xsl:copy>
</xsl:template>
