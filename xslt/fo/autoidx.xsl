...
<xsl:template name="generate-index">
  <xsl:param name="scope" select="(key('id', $rootid)|/)[last()]"/><!-- Replaced "ancestor::book" with the key() expression-->
...
<xsl:template name="generate-basic-index">
  ...
  <!-- Select either the node given by the key() function (usually book) or the root node -->
  <xsl:variable name="terms" select="(key('id', $rootid) | /)[last()]//indexterm
                        [count(.|key('letter',
  ...
  <!--<xsl:message>
    ...
    others          = <xsl:value-of select="$others"/>
    object type   = <xsl:value-of select="exslt:object-type($terms)"/>
  </xsl:message>-->
