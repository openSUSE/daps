    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:element name="{$tag}" namespace="http://www.w3.org/1999/xhtml">
          ...
          <!-- toms: Here is the change: put $graphics.path in front of $filename -->
<!--          <xsl:message>
           graphics.path = "<xsl:value-of select="$graphics.path"/>
          </xsl:message>//-->
            <xsl:choose>
              <xsl:when test="$graphics.path">
                <xsl:value-of select="concat($graphics.path, $filename)"/>
              </xsl:when>
              <xsl:otherwise>
